        ...
        for (int i = 0; i < 256; ) {
            ...
            if (fields > 2) {
                ...
                if (stream_id >= streamCount) {
                    throw new IOException("Illegal stream id value " + stream_id + " must be < " + streamCount);
                }
            }
            if (fields > 3) {
                size = in.readVarInt();
            ...
            if (stream_id >= streamCount) {
                throw new IOException(String.format("Invalid stream value %d, must be < %d", stream_id, streamCount));
            }

            if (count <= 0 || (count > 256 - i - (i <= 'N' ? 1 : 0))) {
                throw new IOException(String.format(
                        "Invalid count value %d, must be > 0 && < %d", count, 256 - i - (i <= 'N' ? 1 : 0)));
            }

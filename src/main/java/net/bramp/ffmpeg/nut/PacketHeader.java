...
public class PacketHeader {
    ...
    public void read(NutDataInputStream in, long startcode) throws IOException {
        ...
        if (forwardPtr > 4096) {
            ...
            if (checksum != expected) {
                ...
                throw new IOException(String.format("invalid header checksum %X want %X", expected, checksum));
            }
        }
    ...
    public String toString() {
        return "PacketHeader{startcode=" + Packet.Startcode.toString(startcode) + ", forwardPtr=" + forwardPtr
                + (forwardPtr > 4096 ? ", checksum=" + checksum : "") + '}';
    }
}
